    ...
    def is_valid_steam_id(text: str) -> bool:
        """Check if text is a valid Steam App ID (numeric)."""
        if not text or not text.isdigit():
            return False
        steam_id = int(text)
        # Steam App IDs are typically positive and reasonable in size
        return steam_id > 0 and steam_id < 10000000
    ...
    async def send_instructions_if_missing():
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "Just type `<steam_id>` – Quick gen without the command!\n"
    ...
    async def on_message(message):
        ...
        # Handle direct Steam ID input in gen channel
        if (message.channel.id == GEN_CH and 
            not message.content.startswith('/') and 
            not message.content.startswith('!') and
            len(message.content.strip()) > 0):
            
            # Extract potential Steam ID from message
            potential_steam_id = message.content.strip().split()[0]  # Take first word
            
            if is_valid_steam_id(potential_steam_id):
                ...
                mock_interaction = MockInteraction(message.author, message.channel.id)
                await process_gen_request(mock_interaction, potential_steam_id, message.channel)
    ...
    async def process_gen_request(interaction, steamid: str, channel=None):
        """Process a gen request (either from slash command or direct message)."""
        if interaction.channel_id != GEN_CH:
            if hasattr(interaction, 'response'):
                ...
                return await channel.send("❌ Use this command in the gen channel.")
        ...
        if not limit_ok(interaction.user, role_ids):
            if hasattr(interaction, 'response'):
                ...
                return await channel.send("❌ You reached your daily limit.")
        ...
        if not steam:
            if hasattr(interaction, 'response'):
                ...
                return await channel.send("❌ Steam ID not found.")
        ...
        if not deathstruck_link:
            ...
            try:
                ...
                print(f"[WARN] Auto-request post failed: {e}")
            ...
            if hasattr(interaction, 'response'):
                ...
                return await channel.send(f"❌ **{steam.get('name','Unknown')}** is not available. We've been notified and will check availability.")
        ...
        if hasattr(interaction, 'response'):
            await interaction.response.send_message("✅ Manifest posted and sent to your DMs (if open).", ephemeral=True)
        else:
            await channel.send("✅ Manifest posted and sent to your DMs (if open).")
    ...
    async def gen(interaction: discord.Interaction, steamid: str):
        await process_gen_request(interaction, steamid)
